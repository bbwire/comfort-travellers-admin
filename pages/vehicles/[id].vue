<template>
  <div class="space-y-6" v-if="vehicle">
    <header class="flex flex-col gap-2">
      ...
      <div>
        ...
        <h1 class="text-3xl font-semibold text-gray-900">Edit {{ vehicle.vehicleNumber }}</h1>
        <p class="text-sm text-gray-500">Update availability, crew, or maintenance details.</p>
      </div>
    </header>
    ...
    <form class="space-y-6 rounded-3xl border border-gray-200 bg-white/90 p-6 shadow-sm backdrop-blur" @submit.prevent="handleSubmit">
      ...
      <div>
        ...
        <p class="text-xs text-gray-500">Add driver and conductor names.</p>
      ...
      <div class="flex flex-wrap gap-3">
        ...
        >
          {{ vehiclesStore.loading ? 'Saving…' : 'Update vehicle' }}
        </button>
        <NuxtLink
          to="/vehicles"
  ...
  <div v-else class="rounded-3xl border border-gray-200 bg-white/90 p-8 text-center text-sm text-gray-500">
    Loading vehicle…
  </div>
</template>
...
const vehicle = ref(vehiclesStore.currentVehicle)
const form = reactive({
  vehicleNumber: '',
  status: 'online',
  crew: [] as string[],
...
const syncForm = () => {
  ...
  form.vehicleNumber = vehicle.value.vehicleNumber
  form.status = vehicle.value.status
  form.notes = vehicle.value.notes || ''
  form.isActive = vehicle.value.isActive
  crewInputs.value = vehicle.value.crew?.length ? [...vehicle.value.crew] : ['']
}

const addCrew = () => crewInputs.value.push('')

const removeCrew = (index: number) => {
  crewInputs.value.splice(index, 1)
  if (!crewInputs.value.length) crewInputs.value.push('')
}
...
const handleSubmit = async () => {
  ...
  const result = await vehiclesStore.updateVehicle($firebase.firestore, vehicleId, validation.data)
  if (!result.success) {
...
onMounted(async () => {
  const vehicleId = route.params.id as string
  const loaded = await vehiclesStore.fetchVehicleById($firebase.firestore, vehicleId).catch(() => null)
  if (!loaded) {
    router.push('/vehicles')
    return
  }
  vehicle.value = loaded
  syncForm()
})
</script>
