<template>
  <div class="space-y-6">
    <header>
      ...
      <h1 class="mt-2 text-3xl font-semibold text-gray-900">Edit Trip</h1>
      <p class="text-sm text-gray-500">Update departure details, vehicles, and seat availability.</p>
    </header>
...
const loadTrip = async () => {
  try {
    await tripsStore.fetchTripById($firebase.firestore, route.params.id as string)
    if (!tripsStore.currentTrip) {
      router.push('/trips')
      return
    }
    Object.assign(formData, {
      title: tripsStore.currentTrip.title,
      routeId: tripsStore.currentTrip.routeId,
      vehicleId: tripsStore.currentTrip.vehicleId,
      vehicleNumber: tripsStore.currentTrip.vehicleNumber || '',
      departureTime: tripsStore.currentTrip.departureTime,
      arrivalTime: tripsStore.currentTrip.arrivalTime,
      totalSeats: tripsStore.currentTrip.totalSeats,
      seatsBooked: tripsStore.currentTrip.seatsBooked,
      status: tripsStore.currentTrip.status,
      notes: tripsStore.currentTrip.notes || '',
      availableSeatsText: tripsStore.currentTrip.availableSeats?.join(', ') || '',
    })
  } catch {
    router.push('/trips')
  }
...
const handleSubmit = async () => {
  ...
  try {
    const result = await tripsStore.updateTrip(
      $firebase.firestore,
...
watch(
  ...
  (vehicleId) => {
    ...
    formData.vehicleNumber = selected?.vehicleNumber || formData.vehicleNumber || ''
  }
)
</script>
